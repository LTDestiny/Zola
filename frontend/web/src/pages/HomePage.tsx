  ...
  return (
    <div className="min-h-screen grid grid-rows-[auto_1fr_auto]">
      <header className="h-14 bg-white border-b border-slate-200 px-4 md:px-8 flex items-center gap-4">
        <div className="text-zola-blue text-4xl font-bold leading-none tracking-tight">Zola</div>
        <nav className="ml-auto hidden lg:flex items-center gap-6">
          {menuItems.map((item) => (
            <a href="#" key={item} className="text-xs font-bold text-slate-800 hover:text-zola-blue transition-colors">
              {item}
            </a>
          ))}
        </nav>
      ...
      <main className="p-4 md:p-8 grid place-items-center">
        <section className="w-full max-w-6xl rounded-xl bg-white shadow-card grid lg:grid-cols-2 gap-8 p-6 md:p-10">
          <div>
            <h1 className="text-3xl md:text-5xl fonst-bold leading-tight text-slate-900">
              Tai Zola PC cho may tinh
            </h1>
            <h2 className="mt-3 text-lg md:text-2xl font-semibold text-slate-800">
              Ung dung Zola PC da co mat tren Windows, Mac OS, Web
            </h2>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <a
                className="h-11 px-6 rounded-lg bg-zola-blue text-white inline-flex items-center justify-center font-semibold hover:bg-blue-700 transition-colors"
                href="#"
              >
                Tai ngay
              </a>
              <Link
                className="h-11 px-6 rounded-lg border border-zola-blue text-zola-blue inline-flex items-center justify-center font-semibold hover:bg-blue-50 transition-colors"
                to="/login"
              >
                Dung ban web
              </Link>
      ...
      <footer className="h-12 bg-white border-t border-slate-200 grid place-items-center px-4 text-center text-xs text-slate-600">
        <p className="truncate md:whitespace-normal">
          © 2012 - 2026 Mot san pham cua Zola Group · Dieu khoan su dung dich vu
