...
import { registerWithEmail, toErrorMessage, verifyRegisterOtp } from "../api/authApi";
  ...
  return (
    <main className="min-h-screen px-4 py-8 md:py-12 bg-zalo-sky flex flex-col items-center gap-4">
      ...
      <section className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white shadow-card overflow-hidden">
        <header className="px-6 py-4 border-b border-slate-200 text-center text-2xl font-semibold text-slate-900">{t("register")}</header>

        <div className="p-6 grid gap-4">
          <label className="h-12 border-b border-slate-200 flex items-center">
            <input className="w-full border-none outline-none text-sm" type="text" placeholder={t("fullName")} value={fullName} onChange={(e) => setFullName(e.target.value)} />
          </label>
          ...
          <label className="h-12 border-b border-slate-200 flex items-center">
            <input className="w-full border-none outline-none text-sm" type="password" placeholder={t("password")} value={password} onChange={(e) => setPassword(e.target.value)} />
          </label>

          <label className="h-12 border-b border-slate-200 flex items-center">
            <input className="w-full border-none outline-none text-sm" type="password" placeholder={t("confirmPassword")} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
          </label>
          ...
          <Link to="/policy" className="text-sm text-zalo-blue hover:text-blue-700 text-center">
            {t("readPolicy")}
          </Link>

          <button className="h-11 rounded-lg bg-zalo-blue text-white font-semibold hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" type="button" onClick={() => void onRegister()} disabled={loading || !acceptedPolicy || !email.trim() || !fullName.trim() || !password.trim() || !confirmPassword.trim()}>
            {t("createAccount")}
          </button>
          ...
          {otpRequired && (
            <>
              <label className="h-12 border-b border-slate-200 flex items-center">
                <input className="w-full border-none outline-none text-sm" type="text" placeholder={t("otpCode")} value={otp} onChange={(e) => setOtp(e.target.value)} />
              </label>

              <button className="h-11 rounded-lg border border-zalo-blue text-zalo-blue font-semibold hover:bg-blue-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" type="button" onClick={() => void onVerifyRegisterOtp()} disabled={loading || !otp.trim()}>
                {t("verifyRegister")}
              </button>
            </>
          )}
