...
import { ChatRealtimeClient, type ChatRealtimeEvent } from "../api/chatRealtime";
...
export function ChatPage() {
    ...
    const fetchConversations = async () => {
        try {
            ...
            } else if (activeConversationId && !items.some((conversation) => conversation.id === activeConversationId)) {
        ...
        const client = new ChatRealtimeClient(accessToken, {
            ...
            onEvent: (event: ChatRealtimeEvent) => {
                ...
                const normalizedMessage: MessageItem = {
                    ...
                    type: payload.type,
                    content: payload.recalled ? "This message was recalled" : payload.content,
                ...
                const isDeletedForMe = payload.deletedForUsers?.includes(myUserIdRef.current ?? "");
                if (isDeletedForMe) {
                ...
                if (event.conversationId === activeConversationIdRef.current) {
                    setMessages((prev) => {
                        ...
                        if (exists) {
                            return prev.map((item) => item.id === normalizedMessage.id ? normalizedMessage : item);
                        }
    ...
    useEffect(() => {
        ...
        const loadMessages = async () => {
            try {
                ...
                const result = await getMessages(activeConversationId, { page: 0, size: 100 });
    ...
    const onSendMessage = async () => {
        ...
        try {
            setIsSending(true);
            const result = await sendMessage(activeConversationId, content, { type: "TEXT" });
            setMessages((prev) => {
    ...
    const onSendFile = async (file: File) => {
        ...
        try {
            ...
            const sizeLimit = typeByMime === "IMAGE"
                ? MAX_IMAGE_BYTES
                : typeByMime === "VIDEO"
                    ? MAX_VIDEO_BYTES
                    : MAX_FILE_BYTES;
    ...
    const onRecallMessage = async (messageId: string) => {
        ...
        try {
            const realtimeSent = Boolean(
                realtimeClientRef.current?.publishRecall(activeConversationId, messageId),
            );
    ...
    const onForwardMessage = async (messageId: string) => {
        if (!activeConversationId) return;
        const targets = conversations.filter((conversation) => conversation.id !== activeConversationId);
        if (targets.length === 0) {
            setBannerMessage(language === "vi" ? "Khong co hoi thoai de chuyen tiep" : "No target conversation to forward");
            return;
        }
    ...
    const onConfirmForwardTargets = async ({
        ...
        try {
            ...
            if (uniqueTargetIds.length === 0) {
                setBannerMessage(language === "vi" ? "Khong co doi tuong hop le de chuyen tiep" : "No valid target to forward");
                return;
            }

            const forwardOne = async (targetConversationId: string) => {
                await forwardMessage(activeConversationId, forwardMessageId, targetConversationId);
                return { targetConversationId, channel: "rest" as const };
            };

            const results = await Promise.allSettled(uniqueTargetIds.map((targetId) => forwardOne(targetId)));

            const successItems = results.filter((item) => item.status === "fulfilled") as PromiseFulfilledResult<{ targetConversationId: string; channel: "rest" }>[];
            const failedItems = results.filter((item) => item.status === "rejected") as PromiseRejectedResult[];
            ...
            if (successCount > 0) {
                ...
                const primaryTargetId = successItems[0]?.value.targetConversationId ?? null;
    ...
    const onReactMessage = async (messageId: string, emoji: string) => {
        ...
        try {
            ...
            setMessages((prev) => prev.map((item) => {
                ...
                const filtered = reactions.filter((value) => !value.startsWith(`${myUserIdRef.current}|`));
                return {
                    ...item,
    ...
    const onAddFriend = async () => {
        ...
        try {
            ...
            const conversationResult = await createDirectConversation(friendProfile.id);
            await fetchConversations();
            setActiveConversationId(conversationResult.data.id);
            setBannerMessage(language === "vi" ? "Da tao hoi thoai" : "Conversation created");
    ...
    const canAddFriend =
        Boolean(friendProfile) && friendshipStatus !== "PENDING" && friendshipStatus !== "ACCEPTED";
    ...
    const reloadConversationMessagesWithRetry = async (conversationId: string, attempts = 4) => {
        for (let index = 0; index < attempts; index += 1) {
            try {
                const result = await getMessages(conversationId, { page: 0, size: 100 });
    ...
    const messageBadge = conversations.length > 0 ? Math.min(conversations.length, 9) : 0;
                ...
                <aside className="w-[320px] shrink-0 border-r border-slate-200 bg-white">
                    {activeTab === "contacts" && (
                        <div className="flex h-full flex-col">
                                ...
                                {friendProfile ? (
                                    <div className="rounded-xl border border-slate-200 bg-white p-3">
                                        <div className="mb-2 flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-semibold text-slate-800">{friendProfile.fullName}</p>
                                                <p className="text-xs text-slate-500">{friendshipStatus}</p>
                                            </div>
                                        </div>
                                        <button
                                            type="button"
                            ...
                            <div className="p-4 pb-2">
                                <h2 className="text-sm font-semibold text-slate-800">{language === "vi" ? "Tat ca ban be" : "All Friends"}</h2>
                            </div>
                            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-3">
                                {contactUsers.map((user) => (
                                    <div key={user.id} className="mb-1 flex items-center justify-between rounded-xl p-3 transition-all duration-200 hover:bg-slate-50">
                    ...
                    {activeTab === "profile" && (
                        <div className="p-5">
                            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                ...
                                <h2 className="text-base font-semibold text-slate-800">{myProfile?.fullName ?? "User"}</h2>
                                <p className="text-xs text-slate-500">{myProfile?.email ?? "-"}</p>
                            </div>

                            <div className="mt-4 space-y-2">
                                <button type="button" className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm text-slate-700 transition-all duration-200 hover:bg-slate-50">
                                    <Settings size={16} />
                                    <span>{language === "vi" ? "Cai dat tai khoan" : "Account Settings"}</span>
                                </button>
                                <button type="button" className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm text-slate-700 transition-all duration-200 hover:bg-slate-50">
                                    <Bell size={16} />
                                    <span>{language === "vi" ? "Thong bao" : "Notifications"}</span>
                                </button>
                                <button type="button" className="flex w-full items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 text-left text-sm text-slate-700 transition-all duration-200 hover:bg-slate-50">
                                    <Shield size={16} />
                    ...
                    {activeTab === "calls" && (
                        <div className="flex h-full items-center justify-center p-6 text-center">
                            <div>
                                <h2 className="text-lg font-semibold text-slate-800">{language === "vi" ? "Cuoc goi" : "Calls"}</h2>
                    ...
                    {activeTab === "settings" && (
                        <div className="p-5">
                            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                                <h2 className="text-base font-semibold text-slate-800">{language === "vi" ? "Cai dat" : "Settings"}</h2>
                                <p className="mt-1 text-xs text-slate-500">{language === "vi" ? "Tuy chinh tai khoan va ung dung" : "Customize account and app preferences"}</p>
                            </div>
                            <div className="mt-4">
                                <Link
                                    to="/login"
            ...
            <main className="min-w-0 flex-1 bg-slate-50">
                {activeTab === "messages" ? (
                    ...
                    <div className="flex h-full items-center justify-center p-8 text-center">
                        <div>
                            <h2 className="text-2xl font-semibold text-slate-800">{language === "vi" ? "Chon tab Messages" : "Select Messages tab"}</h2>
                            <p className="mt-2 text-sm text-slate-500">
                                {language === "vi"
